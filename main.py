...
from getdpi import getapi
#免注册大漠
nreg()
#检测dpi
print('检测到本机系统缩放率为:',int(getapi()),'%,请设置为125% 不要动鼠标 并保持游戏窗口前置')
fixdpi = (1/getapi())*100
#准备调用
...
print('查找\"登录\"按钮中')
while ret == '-1|-1|-1' :
    ret = dm.FindpicE(0, 0, 1535, 863, '01login.bmp', '050505', 0.9, 0)
    time.sleep(1)
pos = ret.split('|')
dm.moveto(float(pos[1])*fixdpi, float(pos[2])*fixdpi)
time.sleep(2)
dm.LeftClick()
ret = '-1|-1|-1'
#3 关闭公告栏
...
#移动窗口
print('移动窗口中 ')
k = dm.FindWindowByProcess('Client.exe','HwndWrapper','MainWindow')
dm.MoveWindow(k,0,0)

for i in range(1,10,1) :
    #5 创建房间
    print('查找\"创建房间\"按钮中,确保游戏框在屏幕左上')
    ret = '-1|-1|-1'
    while ret == '-1|-1|-1':
        ret = dm.FindpicE(0, 0, 1920, 1080, '04jianfang.bmp', '050505', 1, 0)
        time.sleep(1)
    pos = ret.split('|')
    dm.moveto(float(pos[1])*fixdpi, float(pos[2])*fixdpi)
    time.sleep(2)
